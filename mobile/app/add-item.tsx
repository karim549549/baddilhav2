...
export default function AddItemScreen() {
  ...
  const handleSaveItem = () => {
    ...
    if (!formData.itemName.trim() || !formData.description.trim() || !formData.category || !formData.condition) {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 2:
        return <CategoryAndConditionStep formData={formData} updateFormData={updateFormData} />;
      case 3:
      ...
      <View className="flex-row justify-between">
        ...
        {currentStep < totalSteps ? (
          ...
          <TouchableOpacity
            ...
            className="flex-1 bg-pink-500 rounded-xl py-4 ml-2"
          >
            <Text className="text-white text-center font-semibold">Save Item</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
...
// Step 1: Photos Only
const PhotosStep: React.FC<{ formData: FormData; updateFormData: (field: keyof FormData, value: any) => void }> = ({ formData, updateFormData }) => {
      ...
      <View className="bg-blue-50 rounded-xl p-4 mt-6">
        <View className="flex-row items-start">
          ...
          <View className="ml-3 flex-1">
            <Text className="text-blue-800 font-semibold text-sm mb-1">Photo Tips:</Text>
            <Text className="text-blue-700 text-xs leading-4">
              • Take photos in good lighting{'\n'}
              • Show the item from different angles{'\n'}
              • Include any defects or wear clearly{'\n'}
              • First photo should be the best view
            </Text>
          </View>
        </View>
      </View>
...
const CategoryAndConditionStep: React.FC<{ formData: FormData; updateFormData: (field: keyof FormData, value: any) => void }> = ({ formData, updateFormData }) => {
  const categories = [
  ...
  return (
    <View className="p-4">
              ...
              <View className="flex-row items-center">
                ...
                <Text 
                  className={`ml-2 font-semibold text-sm ${
                    formData.category === cat.key ? 'text-pink-500' : 'text-gray-700'
                  }`}
                >
                  {cat.label}
                </Text>
      ...
      {/* Condition Selection */}
      <View className="bg-white rounded-xl p-6 mb-6">
        <Text className="text-gray-800 font-bold text-lg mb-4">Condition *</Text>
        <View className="flex-row flex-wrap">
          {conditions.map((cond) => (
            <TouchableOpacity
              key={cond.key}
            ...
            >
              <Text 
                className={`font-semibold text-sm ${
                  formData.condition === cond.key ? 'text-pink-500' : 'text-gray-700'
                }`}
              >
                {cond.label}
              </Text>
...
const AdditionalDetailsStep: React.FC<{ formData: FormData; updateFormData: (field: keyof FormData, value: any) => void }> = ({ formData, updateFormData }) => {
  return (
    <View className="p-4">
      <View className="bg-white rounded-xl p-6 mb-6">
        <Text className="text-gray-800 font-bold text-lg mb-4">Additional Details</Text>
...
const ReviewAndSaveStep: React.FC<{ formData: FormData }> = ({ formData }) => {
  const getCategoryLabel = (category: ItemCategory | null) => {
    ...
    return Object.values(ItemCategory)[Object.values(ItemCategory).indexOf(category)];
  };

  const getConditionLabel = (condition: ItemCondition | null) => {
    if (!condition) return 'Not selected';
    return Object.values(ItemCondition)[Object.values(ItemCondition).indexOf(condition)];
  };

  return (
    <View className="p-4">
      <View className="bg-white rounded-xl p-6 mb-6">
        <Text className="text-gray-800 font-bold text-lg mb-4">Review Your Item</Text>
        ...
        <View className="space-y-4">
          ...
          <View className="border-b border-gray-200 pb-3">
            ...
            <Text className="text-gray-800 font-semibold">{formData.itemName || 'Not provided'}</Text>
          </View>
          
          <View className="border-b border-gray-200 pb-3">
            <Text className="text-gray-600 text-sm">Description</Text>
            <Text className="text-gray-800 font-semibold">{formData.description || 'Not provided'}</Text>
          </View>
          
          <View className="border-b border-gray-200 pb-3">
            <Text className="text-gray-600 text-sm">Category</Text>
            <Text className="text-gray-800 font-semibold">{getCategoryLabel(formData.category)}</Text>
          </View>
          
          <View className="border-b border-gray-200 pb-3">
            <Text className="text-gray-600 text-sm">Condition</Text>
            <Text className="text-gray-800 font-semibold">{getConditionLabel(formData.condition)}</Text>
          </View>
          
          <View className="border-b border-gray-200 pb-3">
            <Text className="text-gray-600 text-sm">Estimated Value</Text>
            <Text className="text-gray-800 font-semibold">
              {formData.estimatedValue ? `$${formData.estimatedValue}` : 'Not provided'}
            </Text>
          </View>
          
          <View className="border-b border-gray-200 pb-3">
            <Text className="text-gray-600 text-sm">Brand</Text>
            <Text className="text-gray-800 font-semibold">{formData.brand || 'Not provided'}</Text>
          </View>
          
          <View className="border-b border-gray-200 pb-3">
            <Text className="text-gray-600 text-sm">Year</Text>
            <Text className="text-gray-800 font-semibold">{formData.year || 'Not provided'}</Text>
          </View>
          
          <View className="pb-3">
            <Text className="text-gray-600 text-sm">Tags</Text>
            <Text className="text-gray-800 font-semibold">{formData.tags || 'Not provided'}</Text>
          </View>
        </View>
      </View>
      ...
      <View className="bg-blue-50 rounded-xl p-4 mb-6">
        <View className="flex-row items-start">
          ...
          <Text className="text-blue-800 text-sm ml-2 flex-1">
            Review all the information above. Once you save, your item will be available for other users to see and potentially match with.
          </Text>
        </View>
      </View>
    </View>
